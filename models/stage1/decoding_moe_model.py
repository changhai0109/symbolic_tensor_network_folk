...
from .decoding_model import group_query_attention, transformer_decoders
...
def feed_forward_network(symbol_map_value):
    ...
    moe_frame = ReplicateGraph.apply(
        TensorGraph.load_tensor_graph(moe_frame_path),
        "moe.%s",
        old_symbol_map_new_symbol={"Seq": "1"},
    )
...
def transformer_decoder_block(symbol_map_value):
    ...
    input_layernorm = ReplicateGraph.apply(
        ...
        "input_norm.%s",
        old_symbol_map_new_symbol={"tp": "tp", "Seq": "1"},
    )
    mha = ReplicateGraph.apply(
        group_query_attention("tpsp_decoding"),
        "mha.%s",
...
def decoding_moe(num_layers, symbol_map_value, regenerate=False):
    ...
    cache_filename = os.path.join(
        CACHE_DIR, f"decoding_moe_{num_layers}_{int(experts_each_group)}.csv"
    )
    ...
    in_emb = ReplicateGraph.apply(
        ...
        old_symbol_map_new_symbol={
            "Din": "Dvocal", "Dout": "Dmodel", "tp": "tp", "Seq": "1",
        },
    )
    out_emb = ReplicateGraph.apply(
        ...
        "out_emb.%s",
        old_symbol_map_new_symbol={
            "Din": "Dmodel", "Dout": "Dvocal", "tp": "tp", "Seq": "1",
        },
    )
